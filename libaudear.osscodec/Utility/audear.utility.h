#ifndef __AUDEAR_UTILITY_H__
#define __AUDEAR_UTILITY_H__
...
inline void cache_chunk_position ( AEBaseStream * stream, std::vector<POSITIONSIZE> & cache, int * channels, int * samplerate )
{
	uint8_t readBuffer [ 12 ];
	int64_t readed;
	
	int64_t length;
	if ( FAILED ( stream->getLength ( &length ) ) )
		return;

	do
	{
		int64_t position;
		if ( FAILED ( stream->getPosition ( &position ) ) )
			break;

		if ( position >= length )
			break;

		if ( FAILED ( stream->read ( readBuffer, 12, &readed ) ) )
			return;
		...
		if ( size != 0 )
		{
			...
			stream->seek ( kAESTREAMSEEK_CUR, size - 12, nullptr );
		}
		else
		{
